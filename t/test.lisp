...
(defclass recording-csv-parser (io.github.cl-sdk.ini:csv-parser)
  ((events :initform '() :accessor recording-csv-parser-events)))

(defmethod io.github.cl-sdk.ini:csv-parser-begin-document ((parser recording-csv-parser))
  (push '(:begin-document nil) (recording-csv-parser-events parser)))

(defmethod io.github.cl-sdk.ini:csv-parser-end-document ((parser recording-csv-parser))
  (push '(:end-document nil) (recording-csv-parser-events parser)))

(defmethod io.github.cl-sdk.ini:csv-parser-header ((parser recording-csv-parser) row)
  (push (list :header row) (recording-csv-parser-events parser)))

(defmethod io.github.cl-sdk.ini:csv-parser-line ((parser recording-csv-parser) row)
  (push (list :line row) (recording-csv-parser-events parser)))

(defmethod io.github.cl-sdk.ini:csv-parser-result ((parser recording-csv-parser))
...
(test rfc-4180/rule-1-crlf
  ...
  (let ((input (crlf "aaa,bbb,ccc" "zzz,yyy,xxx")))
    (is (equal
	 ...
	 (io.github.cl-sdk.ini:parse-csv input)))))
...
(test rfc-4180/rule-2-no-trailing-crlf
  ...
  (is (equal
       '(("aaa" "bbb" "ccc") ("zzz" "yyy" "xxx"))
       (io.github.cl-sdk.ini:parse-csv (with-crlf "aaa,bbb,ccc" "zzz,yyy,xxx")))))
...
(test rfc-4180/rule-3-optional-header
  ...
  (let* ((input (crlf "field_name,field_name,field_name"
		      "aaa,bbb,ccc"
		      "zzz,yyy,xxx"))
	 (rows (io.github.cl-sdk.ini:parse-csv input)))
...
(test rfc-4180/rule-4-comma-separator
  ...
  (is (equal '(("aaa" "bbb" "ccc")) (io.github.cl-sdk.ini:parse-csv "aaa,bbb,ccc"))))
...
(test rfc-4180/rule-7-escaped-double-quote
  ...
  (is (equal
       '(("aaa" "b\"bb" "ccc"))
       (io.github.cl-sdk.ini:parse-csv "\"aaa\",\"b\"\"bb\",\"ccc\""))))
...
(test rfc-4180/empty-fields
  ...
  (is (equal '(("" "b" "")) (io.github.cl-sdk.ini:parse-csv ",b,"))))

(test rfc-4180/single-field
  "Single field with no separators"
  (is (equal '(("hello")) (io.github.cl-sdk.ini:parse-csv "hello"))))
...
(test rfc-4180/bare-lf-line-ending
  ...
  (is (equal
       '(("a" "b") ("c" "d"))
       (io.github.cl-sdk.ini:parse-csv (format nil "a,b~%c,d")))))
...
(test rfc-4180/read-custom-separator
  ...
  (is (equal '(("a" "b" "c"))
	     (io.github.cl-sdk.ini:parse-csv (format nil "a~Cb~Cc" #\Tab #\Tab)
			      :separator #\Tab))))
...
(test rfc-4180/write-crlf-terminator
  ...
  (let ((output (io.github.cl-sdk.ini:write-csv '(("aaa" "bbb") ("zzz" "yyy")) nil)))
...
(test rfc-4180/write-quote-comma
  ...
  (let ((output (io.github.cl-sdk.ini:write-csv '(("hello, world" "42")) nil)))
...
(test rfc-4180/write-escape-quote
  ...
  (let ((output (io.github.cl-sdk.ini:write-csv '(("say \"hi\"")) nil)))
...
(test rfc-4180/write-quote-newline
  ...
  (let ((output (io.github.cl-sdk.ini:write-csv (list (list (format nil "line1~%line2"))) nil)))
...
(test rfc-4180/round-trip
  ...
  (let ((data '(("field_name" "field_name" "field_name")
		("aaa" "bbb" "ccc")
		("zzz" "yyy" "xxx"))))
    (is (equal data (io.github.cl-sdk.ini:parse-csv (io.github.cl-sdk.ini:write-csv data nil))))))

;;; always-quote option
(test rfc-4180/write-always-quote
  "always-quote wraps every field regardless of content"
  (let ((output (io.github.cl-sdk.ini:write-csv '(("a" "b")) nil :always-quote t)))
...
(test rfc-4180/write-custom-separator
  ...
  (let ((output (io.github.cl-sdk.ini:write-csv '(("a" "b" "c")) nil :separator #\Tab)))
...
(test rfc-4180/write-to-string
  ...
  (is (typep (io.github.cl-sdk.ini:write-csv '(("a" "b")) nil) 'string)))
...
(test rfc-7111/row-single
  ...
  (let ((table '(("r1c1" "r1c2") ("r2c1" "r2c2") ("r3c1" "r3c2"))))
    (is (equal '(("r1c1" "r1c2")) (io.github.cl-sdk.ini:select-by-fragment table "row=1")))
    (is (equal '(("r2c1" "r2c2")) (io.github.cl-sdk.ini:select-by-fragment table "row=2")))))
...
(test rfc-7111/row-range
  ...
  (let ((table '(("r1") ("r2") ("r3") ("r4") ("r5"))))
    (is (equal '(("r2") ("r3") ("r4")) (io.github.cl-sdk.ini:select-by-fragment table "row=2-4")))))
...
(test rfc-7111/row-open-end-range
  ...
  (let ((table '(("r1") ("r2") ("r3") ("r4"))))
    (is (equal '(("r3") ("r4")) (io.github.cl-sdk.ini:select-by-fragment table "row=3-")))))
...
(test rfc-7111/row-list
  ...
  (let ((table '(("r1") ("r2") ("r3") ("r4") ("r5"))))
    (is (equal '(("r1") ("r3") ("r5"))
	       (io.github.cl-sdk.ini:select-by-fragment table "row=1,3,5")))))
...
(test rfc-7111/col-single
  ...
  (let ((table '(("a" "b" "c") ("1" "2" "3"))))
    (is (equal '(("b") ("2")) (io.github.cl-sdk.ini:select-by-fragment table "col=2")))))
...
(test rfc-7111/col-range
  ...
  (let ((table '(("a" "b" "c" "d" "e") ("1" "2" "3" "4" "5"))))
    (is (equal '(("b" "c" "d") ("2" "3" "4"))
	       (io.github.cl-sdk.ini:select-by-fragment table "col=2-4")))))
...
(test rfc-7111/cell-single
  ...
  (let ((table '(("a" "b" "c")
    ...
    (is (equal '(("" "" "f"))
	       (io.github.cl-sdk.ini:select-by-fragment table "cell=2-3")))))
...
(test rfc-7111/cell-wildcard-row
  ...
  (let ((table '(("a" "b" "c") ("1" "2" "3") ("x" "y" "z"))))
    (is (equal '(("" "b" "") ("" "2" "") ("" "y" ""))
	       (io.github.cl-sdk.ini:select-by-fragment table "cell=*-2")))))
...
(test rfc-7111/cell-multiple-pairs
  ...
  (let ((table '(("a" "b" "c" "d")
    ...
    (is (equal '(("" "b" "" "")
		 ("" "" "" "l"))
	       (io.github.cl-sdk.ini:select-by-fragment table "cell=1-2,3-4")))))
...
(test rfc-7111/include-header
  ...
  (let ((table '(("name" "age" "city")
		 ...
		 ("Bob"   "25" "LA")
		 ("Carol" "35" "SF"))))
    (let ((result (io.github.cl-sdk.ini:select-by-fragment table "row=1,3" :include-header t)))
...
(test rfc-7111/multiple-selectors
  ...
  (let ((r (io.github.cl-sdk.ini:parse-fragment "row=1;col=2")))
    (is (= 2 (length r)))
...
(test rfc-7111/wildcard-star
  ...
  (let ((r (io.github.cl-sdk.ini:parse-fragment "row=*")))
...
(test rfc-7111/open-ended-range
  ...
  (let ((r (io.github.cl-sdk.ini:parse-fragment "row=3-")))
...
(test has-header/default-returns-header-as-second-value
  ...
  (destructuring-bind (header &rest rows)
      (io.github.cl-sdk.ini:parse-csv (with-crlf "name,age" "Alice,30" "Bob,25"))
    (is (= 2 (length rows)))
...
(test has-header/default-primary-value-data-only
  ...
  (destructuring-bind (header &rest rows)
      (io.github.cl-sdk.ini:parse-csv (with-crlf "name,age" "Alice,30"))
...
(test has-header/explicit-t
  ...
  (destructuring-bind (header &rest rows)
      (io.github.cl-sdk.ini:parse-csv (with-crlf "id,val" "1,a") :has-header t)
    (is (= 1 (length rows)))
...
(test has-header/nil-returns-nil-header
  ...
  (destructuring-bind (header &rest rows)
      (io.github.cl-sdk.ini:parse-csv (with-crlf "Alice,30" "Bob,25") :has-header nil)
    (is (= 2 (length rows)))
    (is (null header))))
...
(test has-header/empty-input
  ...
  (destructuring-bind (header &rest rows)
      (io.github.cl-sdk.ini:parse-csv "" :has-header t)
    (is (null rows))
    (is (null header))))
...
(test has-header/single-row-is-header
  ...
  (destructuring-bind (header &rest rows)
      (io.github.cl-sdk.ini:parse-csv "col1,col2,col3" :has-header t)
    (is (null rows))
...
(test has-header/write-csv-no-header
  ...
  (is (string= (io.github.cl-sdk.ini:write-csv '(("Alice" "30") ("Bob" "25")) nil)
...
(test has-header/write-csv-nil-no-header-line
  "write-csv with :headers nil produces no header row"
  (is (string= (io.github.cl-sdk.ini:write-csv '(("a" "b")) nil :headers nil)
	       (crlf "a,b"))))
...
(test has-header/write-csv-header-quoting
  ...
  (let ((output (io.github.cl-sdk.ini:write-csv '(("1" "2")) nil
				  :headers '("col,a" "col b"))))
...
(test sax-parser/custom-parser-instance
  ...
  (let ((parser (make-instance 'recording-csv-parser)))
    (is (equal '((:begin-document nil)
		 ...
		 (:line ("Alice" "30"))
		 (:end-document nil))
	       (io.github.cl-sdk.ini:parse-csv (with-crlf "name,age" "Alice,30") :parser parser)))))

(test sax-parser/empty-input
  "parse-csv still emits document boundary events on empty input"
  (is (equal '((:begin-document nil)
	       (:end-document nil))
	     (io.github.cl-sdk.ini:parse-csv "" :parser (make-instance 'recording-csv-parser)))))
...
(test csv-conditions/parse-error-on-bad-fragment
  ...
  (signals io.github.cl-sdk.ini:csv-parse-error (io.github.cl-sdk.ini:parse-fragment "unknown=1")))

(test csv-conditions/parse-error-message
  "csv-parse-error carries a human-readable message"
  (handler-case (io.github.cl-sdk.ini:parse-fragment "bad=selector")
    (io.github.cl-sdk.ini:csv-parse-error (c)
      (is (typep (io.github.cl-sdk.ini:csv-parse-error-message c) 'string)))))
